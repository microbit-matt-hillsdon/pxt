...
function flyoutCategory(
    workspace: Blockly.WorkspaceSvg,
    useXml = true,
): Element[] | Blockly.utils.toolbox.FlyoutItemInfo[] {
    if (!useXml) {
        return []
    }
        ...
        if (variableModelList.length > 0) {
            ...
            for (let i = 0; i < variableModelList.length; i++) {
                ...
                if (Blockly.Blocks['variables_get']) {
                    ...
                    let block = mkVariableFieldBlock("variables_get", variable.getId(), variable.getType(), variable.getName(), false);
            ...
            if (Blockly.Blocks['variables_change']) {
                ...
                let block = mkVariableFieldBlock("variables_change", mostRecentVariable.getId(), mostRecentVariable.getType(), mostRecentVariable.getName(), false);
            ...
            if (Blockly.Blocks['variables_set']) {
                ...
                let block = mkVariableFieldBlock("variables_set", mostRecentVariable.getId(), mostRecentVariable.getType(), mostRecentVariable.getName(), false);
                block.setAttribute("gap", gap + "")
                {
                    let value = Blockly.utils.xml.createElement('value');
                    value.setAttribute('name', 'VALUE');
