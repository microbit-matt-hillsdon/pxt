...
export abstract class FieldMatrix extends Blockly.Field {
    ...
    protected createMatrixDisplay({
        ...
        for (let y = 0; y < this.numMatrixRows; y++) {
            const row = pxsim.svg.child(this.elt, "g", { 'role': 'row' });
    ...
    private addKeyDownHandler() {
        this.keyDownBinding = Blockly.browserEvents.bind(this.elt, 'keydown', this, (e: KeyboardEvent) => {
            if (!this.selected) {
                return
            }
    ...
    private addBlurHandler() {
        this.blurBinding = Blockly.browserEvents.bind(this.elt, 'blur', this, (_e: FocusEvent) => {
